#!/usr/bin/env python3
"""Build a standalone interactive HTML chart from team_value_daily.csv.

12 daily lines of Superflex total dynasty value (players + picks), one per team.
Chart.js (CDN) + annotation plugin. Because 12 series exceeds any CVD-safe
categorical cap, IDENTITY is carried primarily by interaction: click a legend
entry to isolate a team, hover to highlight a line and read a full ranked
tooltip; color is a secondary aid. Season boundaries (rookie drafts) are marked.
"""
import csv, json, os
from collections import defaultdict

HERE = os.path.dirname(os.path.abspath(__file__))
rows = list(csv.DictReader(open(os.path.join(HERE, "team_value_daily.csv"))))
...
series_total = {rid: [] for rid in order}
series_players = {rid: [] for rid in order}
series_picks = {rid: [] for rid in order}
idx = {d: i for i, d in enumerate(dates)}
for rid in order:
    series_total[rid] = [None] * len(dates)
    series_players[rid] = [None] * len(dates)
    series_picks[rid] = [None] * len(dates)
for r in rows:
...
# 12 hues stepped for the dark surface — validated (dataviz validator, --mode dark:
# all checks pass; the sole 6-8 CVD WARN is legal because identity's primary channel
# is interaction: click-isolate legend + hover-highlight + ranked tooltip).
...
<title>Dynasty Value Over Time — Dynasuiiii (12 teams, Superflex, KTC-backed)</title>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <p class="note">Value source (provenance): player values are KTC Superflex actuals, forward-filled ≤7 days across gaps, or <code>rookie_realized</code> when a converted rookie pick; deep-bench / K / DEF / retired players not in KTC’s dynasty universe count 0. Pick values are KTC pick-tier actuals where a series exists (2026–2028), otherwise <code>pick_backcast</code> estimates (2025, early-2027, 2029) — dashed-era estimates, labeled, never presented as observed KTC. Boundary markers = rookie drafts. Full method & coverage in RESULTS.md.</p>
  </div>
</div>
<script>
  ...
  options:{
    ...
    scales:{
      ...
      y:{grid:{color:css('--grid')}, ticks:{color:css('--ink2'),
         callback:v=>(v/1000)+"k"}, title:{display:true,text:"KTC Superflex value",color:css('--muted')}}
    },
    plugins:{
...
out = os.path.join(HERE, "dynasty_value_over_time.html")
