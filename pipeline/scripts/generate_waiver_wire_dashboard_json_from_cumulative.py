...
def generate_all_transactions_from_cumulative(df: pd.DataFrame, players: Dict, player_values: Dict) -> List[Dict[str, Any]]:
    ...
    # Load historical values from CSV (generated by Stage 5)
    historical_values_map = {}
    csv_file = PIPELINE_DIR / 'waiver_wire_analysis.csv'
    if csv_file.exists():
        try:
            df_csv = pd.read_csv(csv_file)
            # Create lookup: (transaction_id, player_id) -> value_at_transaction
            for _, row in df_csv.iterrows():
                key = (str(row['transaction_id']), str(row['player_id']))
                if pd.notna(row.get('player_value_at_transaction')):
                    historical_values_map[key] = int(row['player_value_at_transaction'])
            logger.info(f"✓ Loaded historical values for {len(historical_values_map)} transactions from CSV")
        except Exception as e:
            logger.warning(f"Could not load historical values from CSV: {e}")
    
    def get_player_value(player_name, transaction_id=None, player_id=None):
        """Get player value, preferring historical value from CSV if available."""
        # Try historical value first if we have transaction_id and player_id
        if transaction_id and player_id:
            key = (str(transaction_id), str(player_id))
            hist_value = historical_values_map.get(key)
            if hist_value is not None:
                return hist_value
        
        # Fallback to current values from asset cache
    ...
    for _, row in df_sorted.iterrows():
        ...
        # Pass transaction_id and player_id for historical value lookup
