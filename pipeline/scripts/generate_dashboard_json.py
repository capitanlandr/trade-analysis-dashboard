...
Converts CSV pipeline outputs to JSON files for the static dashboard.

FILE ARCHITECTURE:
==================
This script transforms CSV data (trades, teams) into JSON format.

Pipeline JSON files (pipeline/*.json):
  - Used for debugging, auditing, and historical records
  - Not read by other scripts (write-only archives)

Dashboard JSON files (dashboard/frontend/public/api-*.json):
  - Consumed directly by React frontend
  - Generated by their respective data source scripts:
    * api-standings.json: Written by fetch_standings.py
    * api-playoff-scenarios.json: Written by simulate_playoff_scenarios.py
    * api-trades.json: Generated by this script from CSVs
    * api-teams.json: Generated by this script from CSVs
    * api-stats-summary.json: Generated by this script from CSVs

This script focuses on CSV-to-JSON transformation, not JSON-to-JSON copying.
"""
...
def generate_json_files():
    ...
    # NOTE: api-standings.json and api-playoff-scenarios.json are written directly
    # by their respective data source scripts (fetch_standings.py and simulate_playoff_scenarios.py)
    ...
    logger.info(f"   (Standings and playoff scenarios written by their source scripts)")
    logger.info("="*80)
