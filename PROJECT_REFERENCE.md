...
- Frontend still reads static JSON files (`USE_STATIC_DATA = true` in `api.ts`).
- Need to wire frontend to read from Lambda API instead of static files.
- Need Dashboard API to read from DynamoDB instead of live Sleeper calls.
